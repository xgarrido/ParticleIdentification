    ...
    int channel_1eNg_cut::_accept()
    {
      ...
      if (is_mode_has_gamma()) {
        if (a_pattern_id != "1eNg") {
          DT_LOG_DEBUG(get_logging_priority(), "Check on existing gammas only applicable to '1eNg' topology !");
          return cuts::SELECTION_INAPPLICABLE;
        }
        ...
        if (ptr_1eNg_pattern->get_number_of_gammas() == 0) {
      ...
      if (is_mode_range_gamma()) {
        if (a_pattern_id != "1eNg") {
          DT_LOG_DEBUG(get_logging_priority(), "Check on number of gammas only applicable to '1eNg' topology !");
          return cuts::SELECTION_INAPPLICABLE;
        }
        const snemo::datamodel::topology_1eNg_pattern * ptr_1eNg_pattern
          = dynamic_cast<const snemo::datamodel::topology_1eNg_pattern *>(&a_pattern);

        const int ngamma = ptr_1eNg_pattern->get_number_of_gammas();
        // if (datatools::is_valid(_number_of_gammas_min_)) {
        if (1) {
        ...
        // if (datatools::is_valid(_number_of_gammas_max_)) {
        if (1) {
