...
namespace snemo {
  ...
  namespace reconstruction {
    ...
    void topology_2eNg_builder::_build_measurement_dictionary(snemo::datamodel::base_topology_pattern & pattern_)
    {
      ...
      for (size_t i_gamma = 1; i_gamma <= pattern_.get_particle_tracks_dictionary().size()-1; ++i_gamma) {
...

        {
          ...
          if (drivers.TOFD) drivers.TOFD->process(e2, gamma,
                                                  ptr_tof->grab_internal_probabilities(),
                                                  ptr_tof->grab_external_probabilities());
        }
...

        {
          ...
          if (drivers.AMD) drivers.AMD->process(e2, gamma, ptr_angle->grab_angle());
        }
